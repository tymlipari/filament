...
class Engine
{
  ...
  Engine._(this._mNativeHandle)
  {
    ...
    _mEntityManager = createEntityManagerFromNative(native.instance.filament_engine_get_entity_manager(_mNativeHandle));
  }
  ...
  RenderBackend get backend => RenderBackend.values.elementAt(native.instance.filament_engine_get_backend(_mNativeHandle));

  swapchain.SwapChain createSwapChain({required Object surface, int flags = 0}) {
    if (!Platform.get().validateSurface(surface)) throw Exception('Invalid surface');
    
    var handle = native.instance.filament_engine_create_swapchain(_mNativeHandle, surface, flags);
    if (handle == nullptr) throw Exception('Failed to create swap chain');
    return swapchain.createSwapChain(handle);
  }

  swapchain.SwapChain createHeadlessSwapChain(int width, int height, int flags) {
    if (width <= 0 || height <= 0) throw Exception('Invalid dimensions');

    var handle = native.instance.filament_engine_create_swapchain_headless(_mNativeHandle, width, height, flags);
    if (handle == nullptr) throw Exception('Failed to create headless swapchain');
    return swapchain.createSwapChain(handle);
  }

  swapchain.SwapChain createSwapChainFromNativeSurface({required NativeSurface surface, int flags}) {
    var handle = native.instance.filament_engine_create_swapchain_from_raw_pointer(_mNativeHandle, surface, flags);
    if (handle == nullptr) throw Exception('Failed to create swap chain from native instance');
    return swapchain.createSwapChain(handle);
  }
  ...
  renderer.Renderer createRenderer() {
    var handle = native.instance.filament_engine_create_renderer(_mNativeHandle);
  ...
  Camera createCamera(int entity) {
    var handle = native.instance.filament_engine_create_camera(_mNativeHandle, entity);
  ...
  Camera getCameraComponent(int entity) {
    var handle = native.instance.filament_engine_get_camera_component(_mNativeHandle, entity);
    if (handle == nullptr) throw Exception('Failed to get camera component for entity');
    return createCameraFromNative(handle, entity);
  }
  ...
  void flushAndWait() => native.instance.filament_engine_flush_and_wait(_mNativeHandle);
