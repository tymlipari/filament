...
class BufferObjectBuilder
{
  ...
  BufferObjectBuilder._() 
    :_mNativeHandle = nullptr 
  {
    _mNativeHandle = native.instance.filament_create_buffer_builder();
    if (_mNativeHandle == nullptr) throw Exception('Failed to create buffer object builder');
  }

  BufferObject build(native.EngineRef engineHandle, int byteSize, BufferBindingType bindingType) {
...
class BufferObject
{
  ...
  BufferObject({required Engine engine, required int byteCount, BufferBindingType binding = BufferBindingType.vertex }) {
